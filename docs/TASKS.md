...
- [ ] **T14.3: 衔接线改「跟時間連」纯时间口径（撤销 T14.2 双闸门）**
  - 背景: 用户实测 2025-01-30「13:45 後沒有連去移動」——驾车 12:42→13:45→13:54→15:00 与 移動 14:00→16:00、驾车 15:15→16:35 并行断开（10→11 重叠 1h/端点 66.7km、11→12 46km 被闸门跳过）；CEO 数据层复现确认 20 段中 9 个被跳过相邻对**全部是 移動↔驾车/步行 跨类型**；用户明确「跟時間連」=时间排序后所有段端点逐点相接（12:42→13:45→13:54→14:00→15:00→15:15→16:00→16:35），与类型/重叠/距离无关
  - 改: `bridgeLines` 撤掉「gapMs≤0 且点距>BRIDGE_OVERLAP_MAX_M→跳过」闸门（T14.2 双闸门撤销），改纯时间口径——所有时间相邻段一律建桥；仅保留「可视端点完全重合」退化对免桥（本就无缝）；删除 `BRIDGE_OVERLAP_MAX_M` 常量；docstring 同步新契约
  - 验收: ①用户示例 12:42→13:45→13:54→14:00→15:00→15:15→16:00→16:35 全连通（含 66.7km/46km 重叠对）；②livedata 2025-01-30 20 段 → 17 处衔接（2 对端点重合免桥）；③T14.2 的双闸门断言翻转/重写（远距重叠对现在必须建桥）；④单测 + build + lint 全绿
  - 关联: DECISIONS「T14.2 双闸门撤销」；DATA-FINDINGS §7 同步改写
