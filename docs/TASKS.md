...
- [ ] **T15: Trip 时间轴视图（纯 GPS 轨迹线）+ 可选"按活动类型"视图**
  - 背景: 用户反馈 T14 系列（bridge lines）仍未解决——他们要的不是虚线桥，而是一条纯时间线：所有 rawSignals（GPS 点）按时间排序连成一条线（单色），停驻点用不同颜色标记，移动点 tooltip 显示 GPS 坐标
  - 改: 默认 Trip 视图改为一行 rawSignals 轨迹线 + visit 标记；无 activity type 着色、无 bridge lines、无 gapMs；可选"按活动类型"视图（切换开关），保留当前 T14.3 行为
  - 验收: ①timeline 模式：rawSignals 单色连续线 + visit 标记 + GPS tooltip；②activity type 模式：当前行为不变；③切换开关可用；④单测 + build + lint 全绿
