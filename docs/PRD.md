...
- v1.13 (2026-09-14): 功能 3 本体重写为与最终实现一致（合并按时间排序 + 去重 + 逐点时间 + 可点链接 + 左栏时间线），并补齐验收条件；**修正 v1.10 遗留的「按本地日分桶」描述**（已被 v1.12 的「按时间排序合并」取代，两版曾自相矛盾）。原因：T16–T17 的 PRD 是**实现后补写**，违反「PRD 先行」，且本体未同步更新 → 记入 retro 行动项