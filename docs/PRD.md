...
- [ ] **功能 3: Trips 视图（行程轨迹）** — 选中日期范围内：绘制行驶路线（activitySegment.waypointPath），**全部段按时间顺序（startMs）排序，相邻段间一律自动补衔接线（浅灰细线/虚线，与实测段视觉可区分，tooltip 标注中间 gap 时长），在地图上形成一条无断口的连续时间线轨迹**；衔接线不因段的类型（移動/驾车/步行…）不同、时间重叠或端点点距远近而跳过——「跟時間連」：时间序列上相邻的段端点逐点相接；保留按交通方式着色（实测段不受衔接线影响）；标出停留点（placeVisit 位置与时长）；渲染有点数上限保护，超限降采样 — 验收: 导入样例数据后选择日期范围，多段行程在地图上形成无断口的连续轨迹（段间断口由衔接线补齐，且衔接线与实测段可辨识），路线 + 停留点标记可见
